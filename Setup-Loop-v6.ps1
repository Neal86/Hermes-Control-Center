...
function Normalize-PluginState {
    ...
    $out = Join-Path $LogDir "plugin-state.out.log"
    $err = Join-Path $LogDir "plugin-state.err.log"
    Remove-Item $out,$err -Force -ErrorAction SilentlyContinue
    $proc = Start-Process -FilePath $python -ArgumentList @($PluginStateHelper,$ConfigPath) -WorkingDirectory $Root -Wait -PassThru -NoNewWindow -RedirectStandardOutput $out -RedirectStandardError $err
    Show-Log $out; Show-Log $err
    return [int]$proc.ExitCode
}
