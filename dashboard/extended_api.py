...
def _load_legacy():
    path = DASHBOARD_ROOT / "plugin_api.py"
    name = "hermes_control_center_legacy_dashboard_api"
    module = sys.modules.get(name)
    if module is not None:
        return module
    spec = importlib.util.spec_from_file_location(name, path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load legacy dashboard API from {path}")
...
def agent_resources(agent: str, refresh: bool = True) -> dict[str, Any]:
    try:
        return {
            "items": ResourceBindings().resources_for_agent(agent, refresh=refresh),
            "fail_closed": True,
        }
    except Exception as exc:
        raise _bad(exc) from exc
